...
class SuratApiController extends Controller
{
    public function index(Request $req)
    {
        ...
        if ($req->filled('klasifikasi_id')) {
            $q->where('klasifikasi_id', $req->klasifikasi_id);
        }
        if ($req->filled('tanggal_diterima')) {
            $q->where('tanggal_diterima', $req->tanggal_diterima);
        }
        if ($req->filled('q')) {
            $qq = $req->q;
    ...
    public function update(Request $req, $id)
    {
        ...
        $data = $req->only(['nomor_surat', 'tanggal_diterima', 'pengirim', 'perihal', 'klasifikasi_id']);
        if ($req->hasFile('file')) {
            if ($s->file && Storage::disk('public')->exists($s->file)) {
                Storage::disk('public')->delete($s->file);
            }
            $path = $req->file('file')->store('surat', 'public');
            $data['file'] = $path;
        }
