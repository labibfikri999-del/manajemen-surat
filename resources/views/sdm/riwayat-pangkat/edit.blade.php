...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-slate-800">Edit Riwayat Pangkat</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <form action="{{ route('sdm.riwayat-pangkat.update', $riwayat->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="golongan" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        @foreach(['I', 'II', 'III', 'IV'] as $g)
                            <option value="{{ $g }}" {{ (old('golongan') ?? $riwayat->golongan) == $g ? 'selected' : '' }}>{{ $g }}</option>
                        @endforeach
                    </select>
                    @error('golongan')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <label class="block text-sm font-bold text-slate-700 mb-2">Ruang <span class="text-red-500">*</span></label>
                     <select name="ruang" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        @foreach(['A', 'B', 'C', 'D', 'E'] as $r)
                            <option value="{{ $r }}" {{ (old('ruang') ?? $riwayat->ruang) == $r ? 'selected' : '' }}>{{ $r }}</option>
                        @endforeach
                    </select>
                    @error('ruang')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="tmt" value="{{ old('tmt') ?? $riwayat->tmt }}" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500">
                    @error('tmt')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
