...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-slate-800">Tambah Riwayat Jabatan</h1>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <form action="{{ route('sdm.riwayat-jabatan.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
                ...
                <div>
                    ...
                    <select name="sdm_master_jabatan_id" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        @foreach($jabatans as $jb)
                            <option value="{{ $jb->id }}" {{ old('sdm_master_jabatan_id') == $jb->id ? 'selected' : '' }}>{{ $jb->nama_jabatan }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-2">Kategori <span class="text-red-500">*</span></label>
                     <select name="kategori" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500">
                        <option value="Fungsional" {{ old('kategori') == 'Fungsional' ? 'selected' : '' }}>Fungsional</option>
                        <option value="Sruktural" {{ old('kategori') == 'Struktural' ? 'selected' : '' }}>Struktural</option>
                        <option value="Non Fungsional" {{ old('kategori') == 'Non Fungsional' ? 'selected' : '' }}>Non Fungsional</option>
                    </select>
                    @error('kategori')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="homebase" value="{{ old('homebase') }}" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" placeholder="Contoh: S1 Admin.Kes">
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="tgl_mulai" value="{{ old('tgl_mulai') }}" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500">
                    @error('tgl_mulai')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="tgl_selesai" value="{{ old('tgl_selesai') }}" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500">
