...
<div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <form action="{{ route('sdm.master-jabatan.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            
            <!-- Nama Jabatan -->
            <div>
                <label class="block text-sm font-bold text-slate-700 mb-2">Nama Jabatan <span class="text-red-500">*</span></label>
                <input type="text" name="nama_jabatan" value="{{ old('nama_jabatan') }}" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" placeholder="Contoh: Kepala Bagian Umum" required>
                @error('nama_jabatan')
            ...
            <div>
                ...
                <input type="number" name="urutan" value="{{ old('urutan', 0) }}" class="w-full rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500" placeholder="0">
            ...
            <div class="pt-6 border-t border-slate-100 flex justify-end gap-3">
                ...
                <button type="submit" class="px-5 py-2.5 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 font-semibold shadow-lg shadow-indigo-200 transition-colors">Simpan</button>
            </div>
        </form>
    </div>
</div>
