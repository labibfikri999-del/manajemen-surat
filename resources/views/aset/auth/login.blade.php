...
<head>
    ...
    <title>Login Aset - YARSI NTB</title>
...
<body class="antialiased text-slate-800 min-h-screen relative flex flex-col items-center justify-center py-12 px-4">
    ...
    <div class="fixed inset-0 w-full h-full -z-10 pointer-events-none overflow-hidden">
        <div class="absolute top-0 -left-4 w-96 h-96 bg-lime-300 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob"></div>
        ...
        <div class="absolute -bottom-32 left-20 w-96 h-96 bg-teal-300 rounded-full mix-blend-multiply filter blur-3xl opacity-50 animate-blob animation-delay-4000"></div>
    </div>
    ...
    <main class="w-full max-w-md relative z-10">
        ...
        <div class="glass-panel rounded-3xl p-8 transform transition-all hover:scale-[1.01] duration-500">
            ...
            <div class="text-center mb-8">
                ...
                <p class="text-slate-500 font-medium">Login Sistem Aset</p>
            </div>
            ...
            @if(session('success'))
            <div class="bg-emerald-50/80 backdrop-blur-sm rounded-xl p-4 mb-6 border border-emerald-100 flex items-center gap-3">
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="username" value="{{ old('username') }}" placeholder="Masukkan username" class="w-full pl-12 pr-4 py-3 rounded-xl bg-white/70 border-2 border-slate-100 focus:border-lime-500 focus:ring-2 focus:ring-lime-100 outline-none transition-all placeholder:text-slate-400 text-slate-800 font-medium" required autofocus>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="password" name="password" id="password" placeholder="••••••••" class="w-full pl-12 pr-12 py-3 rounded-xl bg-white/70 border-2 border-slate-100 focus:border-lime-500 focus:ring-2 focus:ring-lime-100 outline-none transition-all placeholder:text-slate-400 text-slate-800 font-medium" required>
                        <button type="button" onclick="togglePassword()" class="absolute inset-y-0 right-0 pr-4 flex items-center group">
                            <svg id="eyeIcon" class="w-5 h-5 text-slate-400 group-hover:text-lime-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex items-center justify-between pt-2">
                    <label class="flex items-center gap-2 cursor-pointer group">
                        <input type="checkbox" name="remember" {{ old('remember') ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 text-lime-600 focus:ring-lime-500 bg-white/80">
                        <span class="text-sm text-slate-600 group-hover:text-lime-700 transition-colors">Ingat saya</span>
                    </label>
                </div>

                <input type="hidden" name="login_source" value="aset">

                <button type="submit" class="w-full py-3.5 rounded-xl text-white font-bold text-base energy-gradient hover:opacity-90 active:scale-[0.98] transition-all duration-200 shadow-lg shadow-lime-200/50 flex items-center justify-center gap-2 mt-4">
                    <span>Login</span>
    ...
    <a href="{{ url('/') }}" class="fixed bottom-6 left-6 z-50 flex items-center gap-2 px-4 py-2 bg-white/80 backdrop-blur-md border border-slate-200 rounded-full shadow-lg text-slate-600 font-medium text-sm hover:bg-white hover:text-lime-600 hover:scale-105 transition-all duration-300 group">
