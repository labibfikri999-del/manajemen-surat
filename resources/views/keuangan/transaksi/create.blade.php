...
<div class="p-4 md:p-8 max-w-4xl mx-auto">
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
            <div>
                <h1 class="text-xl font-bold text-slate-800">Catat Transaksi Baru</h1>
                <p class="text-slate-500 text-sm">Masukkan detail pemasukan atau pengeluaran.</p>
            </div>
        ...
        <form action="{{ route('keuangan.transaksi.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            
            <!-- Type Selection -->
            <div class="grid grid-cols-2 gap-4">
                <label class="cursor-pointer">
                    <input type="radio" name="type" value="pemasukan" class="peer sr-only" checked>
                ...
                <label class="cursor-pointer">
                    <input type="radio" name="type" value="pengeluaran" class="peer sr-only">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="transaction_date" required value="{{ date('Y-m-d') }}" class="w-full px-4 py-2.5 rounded-xl border-slate-200 focus:border-amber-500 focus:ring focus:ring-amber-200 transition-all">
                </div>
            </div>
            ...
            <!-- Category & Description -->
             ...
             <div>
                ...
                <textarea name="description" rows="3" class="w-full px-4 py-2.5 rounded-xl border-slate-200 focus:border-amber-500 focus:ring focus:ring-amber-200 transition-all" placeholder="Detail transaksi..."></textarea>
            </div>
