...
<head>
    ...
    <title>Sistem Keuangan - YARSI NTB</title>
...
<body class="bg-amber-50/50 text-slate-800 font-sans antialiased">
    <div x-data="{ sidebarOpen: true }" class="flex h-screen overflow-hidden">
        ...
        <!-- Sidebar -->
        <aside :class="sidebarOpen ? 'w-64' : 'w-20'" class="glass-sidebar flex flex-col transition-all duration-300 fixed md:relative z-30 h-full shadow-xl shadow-amber-100/20">
            <!-- Brand -->
            <div class="h-20 flex items-center justify-center border-b border-gray-100/50">
                <img src="{{ asset('images/logo_rsi_ntb_new.png') }}" class="h-10 w-auto" alt="Logo">
                <span x-show="sidebarOpen" class="ml-3 font-bold text-lg text-amber-800 transition-opacity duration-300">Keuangan</span>
            </div>

            <!-- Menu -->
            <nav class="flex-1 overflow-y-auto py-6 space-y-1">
                <!-- Dashboard -->
                <a href="{{ route('keuangan.dashboard') }}" class="{{ request()->routeIs('keuangan.dashboard') ? 'nav-item-active' : 'text-slate-500 hover:text-amber-600 hover:bg-amber-50' }} flex items-center px-6 py-3.5 text-sm font-medium transition-all duration-200 group">
                    <svg class="w-5 h-5 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    <span x-show="sidebarOpen" class="ml-3">Ringkasan</span>
                </a>
                ...
                <!-- Logout -->
                <form method="POST" action="{{ route('logout', ['source' => 'keuangan']) }}" class="mt-8 mb-2">
                    @csrf
            ...
            <div class="p-4 border-t border-gray-100/50">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-amber-100 flex items-center justify-center text-amber-600 font-bold border-2 border-white shadow-sm">
                        {{ substr(Auth::user()->name ?? 'Guest', 0, 1) }}
                    </div>
                    <div x-show="sidebarOpen" class="overflow-hidden">
                        <p class="text-xs font-bold text-slate-700 truncate">{{ Auth::user()->name ?? 'Guest User' }}</p>
                        <p class="text-[10px] text-slate-500 truncate">Finance Officer</p>
                    </div>
                </div>
            </div>
        </aside>
        ...
        <main class="flex-1 overflow-y-auto relative">
            ...
            <button @click="sidebarOpen = !sidebarOpen" class="absolute top-4 left-4 md:hidden p-2 rounded-lg bg-white shadow-sm text-slate-600 z-50">
